...
uint32_t ne::create_renderer(ne::Renderer *renderer, bool enableValidationLayers){
    ...
    ne::create_debug_messenger(renderer, enableValidationLayers);
    return NE_SUCCESS;
}



uint32_t ne::destroy_renderer(ne::Renderer renderer){
...
void ne::create_debug_messenger(ne::Renderer *renderer, bool enableValidationLayers){
    if (!enableValidationLayers) return;
    VkDebugUtilsMessengerCreateInfoEXT createInfo;
    populateDebugMessengerCreateInfo(createInfo);

    if (CreateDebugUtilsMessengerEXT(renderer -> instance, &createInfo, nullptr, &debugMessenger) != VK_SUCCESS) {
